...
<div class="space-y-6">
		...
		>
			...
			<Card.Content class="relative p-4 sm:p-5">
				<div class="flex items-center gap-3 sm:gap-4">
					...
					<div class="min-w-0 flex-1">
						<p class="truncate text-xs font-medium text-slate-500 sm:text-sm">4Ps Beneficiaries</p>
						<p
							class="truncate text-lg font-bold tracking-tight text-slate-900 sm:text-xl lg:text-2xl"
						>
		...
		>
			...
			<Card.Content class="relative p-4 sm:p-5">
				<div class="flex items-center gap-3 sm:gap-4">
					...
					<div class="min-w-0 flex-1">
						<p class="truncate text-xs font-medium text-slate-500 sm:text-sm">Registered Voters</p>
						<p
							class="truncate text-lg font-bold tracking-tight text-slate-900 sm:text-xl lg:text-2xl"
						>
		...
		>
			...
			<Card.Content class="relative p-4 sm:p-5">
				<div class="flex items-center gap-3 sm:gap-4">
					...
					<div class="min-w-0 flex-1">
						<p class="truncate text-xs font-medium text-slate-500 sm:text-sm">Backyard Gardens</p>
						<p
							class="truncate text-lg font-bold tracking-tight text-slate-900 sm:text-xl lg:text-2xl"
						>
	...
	<div class="grid gap-6 lg:grid-cols-2">
		...
		<Card.Root class="border-0 shadow-sm ring-1 ring-slate-200/50">
			<Card.Header>
				<Card.Title class="flex items-center gap-2 text-base">
					<Sprout class="size-5 text-slate-500" />
					Food Security Status
				</Card.Title>
			...
			<Card.Content class="space-y-6">
				<div class="text-center">
					...
					<p class="mt-2 text-sm text-slate-500">{foodSecurityStatus.description}</p>
				</div>
	...
	<Card.Root class="border-0 shadow-sm ring-1 ring-slate-200/50">
		<Card.Header>
			<Card.Title class="flex items-center gap-2 text-base">
				<Syringe class="size-5 text-slate-500" />
				...
				<div class="space-y-4 rounded-lg bg-slate-50 p-4">
					<div class="flex items-center gap-3">
						<div class="rounded-lg bg-orange-100 p-2">
							<Dog class="size-6 text-orange-600" />
						</div>
						<div>
							<p class="font-medium text-slate-900">Dogs</p>
							<p class="text-sm text-slate-500">{formatNumber(social.totalDogs)} total</p>
						</div>
					</div>
					<div class="space-y-2">
						<div class="flex justify-between text-sm">
				...
				<div class="space-y-4 rounded-lg bg-slate-50 p-4">
					<div class="flex items-center gap-3">
						<div class="rounded-lg bg-purple-100 p-2">
							<Cat class="size-6 text-purple-600" />
						</div>
						<div>
							<p class="font-medium text-slate-900">Cats</p>
							<p class="text-sm text-slate-500">{formatNumber(social.totalCats)} total</p>
						</div>
					</div>
					<div class="space-y-2">
						<div class="flex justify-between text-sm">
