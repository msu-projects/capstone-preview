...
>
	<div class="w-full max-w-md space-y-6">
		...
		<div class="text-center">
			...
			<h1 class="mt-4 text-2xl font-bold text-slate-900">South Cotabato Data Bank</h1>
			<p class="mt-2 text-sm text-slate-600">Sign in to access the admin dashboard</p>
		</div>
			...
			<Card.Content class="space-y-2">
				{#each demoAccounts as account}
					<button
						...
						class="w-full rounded-md border border-slate-200 p-2 text-left text-xs transition-colors hover:bg-slate-50"
					>
						<div class="flex items-center justify-between">
							<span class="font-medium text-slate-900">{account.role}</span>
							<span class="rounded bg-slate-100 px-2 py-0.5 text-slate-600">{account.password}</span
							>
						</div>
