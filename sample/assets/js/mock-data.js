/**
 * Mock Data for South Cotabato Convergence Data Bank
...
for (let i = 4; i <= 25; i++) {
  mockSitios.push({
    ...
    barangay: ["Zone 1", "Poblacion", "San Isidro", "Santa Cruz", "San Jose"][
      i % 5
    ],
    municipality: [
      "Koronadal City",
...
// Helper to create phase structure
function createPhases(projectType = 'infrastructure') {
  const commonPhases = [
...
// Helper to create accountability structure
function createAccountability(overrides = {}) {
  const base = {
    project_manager: '',
...
function createMonitoringDetails(overrides = {}) {
  const base = {
    fundSource: "20% LDF",
    fiscalYear: 2025,
...
const mockProjects = [
  {
    ...
    // New milestone-based structure
    phases: createPhases('infrastructure'),
    ...
    delays: [
      {
        ...
        preventive_actions_future: "Add utility coordination checklist to all projects with electrical requirements"
      }
    ],
    ...
    recovery_actions: [
      {
        ...
        description: "Fast-track coordination with SOCOTECO regional office to expedite transformer delivery and permit approval",
      ...
      },
      {
        ...
        description: "Once energized, deploy additional workers for 2-shift operation to compress commissioning timeline",
        owner: "Contractor",
  ...
  },
  {
    ...
    monitoring: createMonitoringDetails({
      ...
      statusSummary: {
        stage: "Ongoing",
        issues:
          "Delayed procurement of roofing materials and adverse weather conditions.",
        recommendations:
  ...
  },
  {
    ...
    monitoring: createMonitoringDetails({
      ...
      statusSummary: {
        ...
        issues:
          "Awaiting approval of supplemental budget and building permits.",
        recommendations:
  ...
  },
  {
    ...
    monitoring: createMonitoringDetails({
      ...
      statusSummary: {
        ...
        issues:
          "Some beneficiaries requested replacement seedlings due to pest infestation.",
        recommendations:
          "Deploy municipal agriculturist to provide pest management clinics.",
      },
      catchUpPlan:
...
for (let i = 6; i <= 20; i++) {
  ...
  const sitio = mockSitios[(i - 1) % mockSitios.length];
  ...
  const actual =
    status === "completed"
      ? 100
      : Math.max(0, Math.min(100, plan + (Math.random() * 8 - 4)));
  ...
  const project = {
    ...
    description: `Community development project focused on ${categories[
      i % 7
    ].toLowerCase()} improvement in ${sitio.name}.`,
...
// ===== STATISTICS =====
const mockStats = {
  total_sitios: mockSitios.length,
  total_projects: mockProjects.length,
  active_projects: mockProjects.filter((p) => p.status === "in_progress")
    .length,
  completed_projects: mockProjects.filter((p) => p.status === "completed")
    .length,
  planning_projects: mockProjects.filter((p) => p.status === "planning").length,
  suspended_projects: mockProjects.filter((p) => p.status === "suspended")
    .length,
  total_beneficiaries: mockProjects.reduce(
    (sum, p) => sum + p.beneficiaries,
    0
  ),
  total_budget: mockProjects.reduce((sum, p) => sum + p.budget, 0),
  average_completion: Math.round(
    mockProjects.reduce((sum, p) => sum + p.completion_percentage, 0) /
      mockProjects.length
  ),
  municipalities: [...new Set(mockSitios.map((s) => s.municipality))].length,
};
...
const mockChartData = {
  projectsByCategory: categories.map((cat) => ({
    category: cat,
    count: mockProjects.filter((p) => p.category === cat).length,
  })),
  projectsByStatus: [
    { status: "Planning", count: mockStats.planning_projects },
    { status: "In Progress", count: mockStats.active_projects },
    { status: "Completed", count: mockStats.completed_projects },
    { status: "Suspended", count: mockStats.suspended_projects },
  ],
  projectsByMunicipality: [
    ...new Set(mockProjects.map((p) => p.municipality)),
  ].map((mun) => ({
    municipality: mun,
    count: mockProjects.filter((p) => p.municipality === mun).length,
  })),
  monthlyProgress: [
...
function getSitioById(id) {
  return mockSitios.find((s) => s.id === parseInt(id));
}
...
/**
 * Get projects by sitio ID
 */
function getProjectsBySitio(sitioId) {
  return mockProjects.filter((p) => p.sitio_id === parseInt(sitioId));
}
...
function filterProjects(filters) {
  return mockProjects.filter((project) => {
    ...
    if (filters.municipality && project.municipality !== filters.municipality)
      return false;
    if (filters.year && project.project_year !== parseInt(filters.year))
      return false;
    if (filters.search) {
 ...
 * Filter sitios
 */
function filterSitios(filters) {
  return mockSitios.filter((sitio) => {
    ...
    if (filters.search) {
      const searchTerm = filters.search.toLowerCase();
      const searchableText =
        `${sitio.name} ${sitio.barangay} ${sitio.municipality}`.toLowerCase();
      if (!searchableText.includes(searchTerm)) return false;
    }
    return true;
  });
